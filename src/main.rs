//! Based on this Embassy example:
//!    https://github.com/embassy-rs/embassy/blob/embassy-rp-v0.8.0/examples/rp235x/src/bin/pio_uart.rs
...
async fn main(_spawner: Spawner) {
    ...
    config.max_packet_size_0 = USB_CTRL_PACKET_SIZE;

    // Create embassy-usb DeviceBuilder using the driver and config.
    // It needs some buffers for building the descriptors.
    let mut config_descriptor = [0; 256];
    let mut bos_descriptor = [0; 256];
    let mut control_buf = [0; USB_CTRL_PACKET_SIZE as usize];
    ...
    // Create classes on the builder - two CDC serial port devices.
    let class0 = CdcAcmClass::new(&mut builder, &mut state0, USB_CDC_PACKET_SIZE);

    let class1 = CdcAcmClass::new(&mut builder, &mut state1, USB_CDC_PACKET_SIZE);
    ...
    let usb_future = async {
        info!("Wait for USB connection");
        usb_rx0.wait_connection().await;
        info!("Connected");
        let _ = join(
            usb_read(&mut usb_rx0, &mut uart_pipe_writer),
            usb_write(&mut usb_tx0, &mut usb_pipe_reader),
        )
        .await;
        info!("Disconnected");
        loop {
